...
description: Bootstrap and verify a Windows Codex development environment for new machines. Use when Codex needs to check what is installed, identify missing or broken tooling, install core developer tools (Node.js, GitHub CLI, ripgrep, uv) with winget, ensure Python virtualenv support and pylint are installed, configure global Git identity, verify GitHub CLI auth, and produce a clear ready/not-ready report.
---
...
6. If Git identity is missing, ask for:
   `user.name` and `user.email`, then set global config.
7. If GitHub auth is missing, run:
   `gh auth login --hostname github.com --git-protocol https --web`.
8. Re-run the audit script and confirm readiness.
